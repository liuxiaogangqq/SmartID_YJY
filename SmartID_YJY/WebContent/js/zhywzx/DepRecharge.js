...
function table() {
	...
	$("#table").datagrid({
		...
		url : "Depuser/select.html", // 发送的url地址
		queryParams :
		// 参数列表
		{
			DepartmentList : $("#sDepartmentInnerId").val(),
		},
		method : 'post',
		idField : 'UserInnerId',
		fitColumns : true,
...
$("#rechargeSubmit").click(function() {
	...
	if (!exp.test($("#rMoney").val())) {
		...
		if (rAfterMoney > 1000000) {
			...
			$.messager.confirm("确认", "是否确认充值[姓名：[<span style='font-size:18px;color:red'>" + $("#rDepartmentName").val() + "</span>],充值金额：[<span style='font-size:18px;color:red'>" + $("#rFavorableMoney").val() + "元</span>]", function(r) {
				if (r) {
...
$("#decreaseSubmit").click(function() {
	...
	if (!exp.test($("#dMoney").val())) {
		...
		if (dAfterMoney < 0) {
			...
			$.messager.confirm("确认", "是否确认减值[姓名：[<span style='font-size:18px;color:red'>" + $("#dDepartmentName").val() + "</span>],减值金额：[<span style='font-size:18px;color:red'>" + $("#dMoney").val() + "元</span>]", function(r) {
				if (r) {
