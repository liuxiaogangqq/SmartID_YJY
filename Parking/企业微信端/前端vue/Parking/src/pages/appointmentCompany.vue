<template>
  <div class="appointment">
    <div class="content">
      <mt-field label="访客姓名" placeholder="请输入来访人姓名" v-model="username"></mt-field>
      <mt-field label="访客单位" placeholder="请输入来访人单位" v-model="company"></mt-field>
      <mt-field label="来访事由" placeholder="请选择" class="visitType" v-model="visitType" readonly @click.native="typeClick"></mt-field>
    ...
    <mt-popup
      class="thingForPopup"
      style="width:100%;"
      v-model="popupTypeVisible"
      position="bottom">
      <mt-picker :slots="slotsType" showToolbar ref="picker">
        <div class="carno-header">
          <mt-button @click="cancelType" class="canael">取消</mt-button>
          <mt-button @click="confirmType" class="confirm">确认</mt-button>
        </div>
      </mt-picker>
    </mt-popup>
    ...
    <!-- <mt-popup
      style="width:100%;"
      v-model="popupCarTypeVisible"
      position="bottom">
      <mt-picker :slots="slotsCarType" value-key="name" @change="onCarTypeChange"></mt-picker>
    </mt-popup> -->
    ...
    <!-- <mt-datetime-picker 
        style="width:100%;" 
        ref="pickerStart" 
        type="datetime" :startDate='startDate' :endDate='endValue' v-model="startValue" @confirm="handleStart">
    </mt-picker> -->
    <!-- <mt-datetime-picker 
        style="width:100%;" 
        ref="pickerEnd" 
        type="datetime" :startDate='startDate' v-model="endValue" @confirm="handleEnd">
    </mt-datetime-picker> -->
    <mt-datetime-picker 
        style="width:100%;" 
        ref="pickerStart" 
  ...
  export default {
    ...
    data() {
      return {
        ...
        slotsType: [{
          flex: 1,
          values: ['商业洽谈', '求职面试', '办理业务', '应邀赴约', '亲友来访','其他'],
          className: 'slot1',
          textAlign: 'center'
        }],
        // slotsHistoryCar: [{
    ...
    computed: {
      slotsCarType(){
        ...
        return slotsCarType1
      }
    },
    created(){
      if(this.$route.query.iswhere=="appointmentCompany"){
      ...
      // 获取车辆类型
      // this.$axios.post("searchCarType",{
      //   requestType: 1
      // })
      //     .then(response => {
      //       if( response.data.status == 200 ){
      //         this.carType = response.data.result
      //       }else{
      //         let instance = Toast({
    ...
    methods: {
      ...
      typeClick() {
        var that = this
        that.popupTypeVisible = true
      },
      cancelType() {
        var that = this
        that.popupTypeVisible = false
      },
      confirmType() {
        var that = this
        that.popupTypeVisible = false
        that.visitType = that.$refs.picker.getValues()[0]
      },
      // 获取历史车牌
      getHistoryCar(){
        var that = this
      ...
      //   var that = this
      //   that.historyCarVisible = false
      //   that.carNumber = that.$refs.pickerCar.getValues()[0]
      // },
      ...
      // carTypeClick() {
      //   var that = this
      //   that.popupCarTypeVisible = true
      // },
      // 车辆类型回显
      // onCarTypeChange(picker, values) {
      //   this.visitCarType = values[0].name;
      //   this.visitCarTypeId = values[0].id;
      // },
      // 预约提交
      appointmentBtn(){
        var that = this
                          ...
                          if( that.endTime1 >= that.startTime1 ){//如果结束时间大于来访时间的15分钟，执行提交
                            ...
                            .then(response => {
                              if( response.data.status == 200 ){
                                ...
                                if( axiosFlag != 1 ){
                                  that.$axios.post("insertCompanyVistor",{
                                    VisitorName: that.username,
                                    VisitorCompany: that.company,
                                    VisitorThing: that.visitType,
                                    // CarTypeInnerId: that.visitCarTypeId,
