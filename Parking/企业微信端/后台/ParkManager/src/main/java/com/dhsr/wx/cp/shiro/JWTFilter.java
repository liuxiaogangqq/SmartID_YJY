...
import com.dhsr.wx.cp.config.ResponseHelper;
...
public class JWTFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        if (isLoginAttempt(request, response)) {
            try {
                ...
                responseError(request, response, PublicResultConstant.UNAUTHORIZED);
            }
        }
        return true;
    }
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if (ComUtil.isEmpty(authorization)) {
            responseError(request, response, PublicResultConstant.UNAUTHORIZEDNULL);
            return false;
        }
    ...
    private boolean verificationPassAnnotation(ServletRequest request, ServletResponse response, HttpServletRequest httpServletRequest, String authorization) throws Exception {
        ...
        System.out.println("----------------------verificationPassAnnotation--------------------");
        ...
        for (String urlMethod : Constant.METHOD_URL_SET) {
            ...
            if (split[0].equals(httpServletRequest.getRequestURI()) && (split[1].equals(httpServletRequest.getMethod()) || split[1].equals("RequestMapping"))) {
                Constant.isPass = true;
                log.info("access to");
                if (ComUtil.isEmpty(authorization)) {
                    //如果当前url不需要认证，则注入当前登录用户时，给一个空的
                    httpServletRequest.setAttribute("currentUser", new Operator());
                    log.info("access to OK");
                    return true;
                } else {
     ...
     * 非法url返回身份错误信息
     */
    private void responseError(ServletRequest request, ServletResponse response, String errDesc) {
        PrintWriter out = null;
        try {
            response.setCharacterEncoding("utf-8");
            out = response.getWriter();
            response.setContentType("application/json; charset=utf-8");
            out.print(JSONObject.toJSONString(ResponseHelper.validationToken(errDesc)));
            //PublicResultConstant.UNAUTHORIZED
            out.flush();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
